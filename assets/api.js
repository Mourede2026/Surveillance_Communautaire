...
function submitSuiviGroupe_(actor, p) {
  if (actor.Role !== ROLES.ASCQ) return { ok: false, error: `Seul un compte ASCQ peut enregistrer un suivi groupé (rôle détecté sur votre session : "${actor.Role || 'vide'}"). Déconnectez-vous puis reconnectez-vous ; si le problème persiste, signalez ce message exact.` };
