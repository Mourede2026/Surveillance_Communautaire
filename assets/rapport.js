 ...
 * - navigation Précédent / Suivant entre les semaines disponibles (la plus récente par défaut)
 ...
 */
let __rapportState = { weeks: [], index: -1 };
...
async function renderRapportModule(containerId) {
  ...
  try {
    ...
    const weeks = rapports.slice().sort((a, b) => (Number(a.Annee) - Number(b.Annee)) || (Number(a.SemaineEpi) - Number(b.SemaineEpi)));
    ...
    __rapportState.index = weeks.length - 1; // la plus récente par défaut
...
async function paintRapport_(containerId) {
  const el = document.getElementById(containerId);
  const { weeks, index } = __rapportState;
  const r = weeks[index];

  el.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      <button class="btn-secondary" id="rapPrev" ${index <= 0 ? 'disabled' : ''}>◀ Semaine précédente</button>
      <strong style="font-size:1.05rem">Semaine ${r.SemaineEpi} / ${r.Annee}</strong>
      <button class="btn-secondary" id="rapNext" ${index >= weeks.length - 1 ? 'disabled' : ''}>Semaine suivante ▶</button>
    </div>
  ...
  try {
    const { detail } = await Api.call('listRapportDetail', { annee: r.Annee, semaine: r.SemaineEpi });
    if (!detail.length) {
